...
spotify_client = spotipy.Spotify(client_credentials_manager=SpotifyClientCredentials(client_id=SPOTIFY_CLIENT_ID,
...
async def help_command(update: Update, context: CallbackContext):
    ...
    help_text = (_(
        "*About bot:*\n"
        "• This bot can search and download music from *Spotify*!\n"
        "• Bot saves downloaded tracks — if track already exist in downloads, bot "
        "will send file *instantly*!\n"
        "• All downloaded tracks available in [this channel](https://t.me/nekotune_db).\n"
        ...
        "• If song name is foreign language (like Japanese), it's advised to write in "
        "that language.\n"
        ...
        "• If you run into problem, try again or DM me! [CLICK](https://t.me/nekohepott)\n"
        "\n"
...
async def search_music(update: Update, context: CallbackContext):
    ...
    for item in tracks_data:
        track_dict = {
            ...
            "youtube_query": f"{item.get('name')} {' '.join(artist.get('name') for artist in item.get('artists', []))}"
        }
...
# display track keyboard with pagination
async def display_tracks(update: Update, context: CallbackContext, user_id):
    ...
    text = _("🔍*Search:* _{query}_\n⬇️*Choose track:*").format(
        query=escape_markdown(query_text, version=2)
    )
    if update.message:
...
async def process_track_selection(update: Update, context: CallbackContext, user_id, track_id):
    ...
    youtube_query = track.get("youtube_query")
    video_url = f"ytsearch:{youtube_query}"
    ...
    try:
        ...
        if isFirstDownload == True:  # if track first time downloaded, send to channel
            ...
            await context.bot.send_audio(
                ...
                #thumb=open(cover_path, 'rb'), pls ignore
                title=track_name,
                performer=artists
            )
            cover_path = None
...
async def download_track(track_name, track_artists, video_url, update, context):
    ...
    user_language = get_user_language(user_id)
    _ = LANGUAGES[user_language].gettext
    log(f"Starting download for track: {track_name}, {video_url}", "INFO")
